...
def convert_to_nunchaku_qwen_image_transformer_block_state_dict(
    state_dict: dict[str, torch.Tensor],
    scale_dict: dict[str, torch.Tensor],
    smooth_dict: dict[str, torch.Tensor],
    branch_dict: dict[str, torch.Tensor],
    ...
    # down_proj_local_name = "img_mlp.net.2.linear"
    # if f"{block_name}.{down_proj_local_name}.weight" not in state_dict:
    #     down_proj_local_name = "img_mlp.net.2"
...
def convert_to_nunchaku_qwen_image_state_dicts(
    state_dict: dict[str, torch.Tensor],
    scale_dict: dict[str, torch.Tensor],
    smooth_dict: dict[str, torch.Tensor],
    branch_dict: dict[str, torch.Tensor],
    ...
    for block_name in block_names:
        convert_fn = convert_to_nunchaku_qwen_image_transformer_block_state_dict
        update_state_dict(
            converted,
            convert_fn(
                state_dict=state_dict,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rank", type=int, default=32, help="rank of the process.")
    ...
    assert "qwen" in model_name.lower(), "Only Qwen-image models are supported."
    ...
    converted_state_dict, other_state_dict = convert_to_nunchaku_qwen_image_state_dicts(
        state_dict=state_dict,
        scale_dict=scale_dict,
        smooth_dict=smooth_dict,
        branch_dict=branch_dict,
    ...
    quantization_config = {
        "method": "svdquant",
        "weight": {
            "dtype": "fp4_e2m1_all" if args.float_point else "int4",
            "scale_dtype": [None, "fp8_e4m3_nan"] if args.float_point else None,
            "group_size": 16 if args.float_point else 64,
        },
        "activation": {
            "dtype": "fp4_e2m1_all" if args.float_point else "int4",
            "scale_dtype": "fp8_e4m3_nan" if args.float_point else None,
            "group_size": 16 if args.float_point else 64,
        },
        "rank": args.rank,
    }
    ...
    safetensors.torch.save_file(state_dict, os.path.join(output_dirpath, f"svdq-int4_r32-{args.model_name}.safetensors"), metadata=meta)
